...
export const getRecommendationPlanList = createAsyncThunk('recommendation-trip-plan',
...
export const getCategoryPlanList = createAsyncThunk('category-trip-plan',
    async (_, { rejectWithValue }) => {
        try {
            const response = await PlanService.getPlanCategory();
            const plan_category_list: HistoryOrPlaceCategryType[] = response.data?.results;
...
export const getTripPlanList = createAsyncThunk('get-trip-plan-list',
...
export const planSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(getRecommendationPlanList.fulfilled, (state, action: PayloadAction<RecommendationType[]>) => {
                state.loading = false
            ...
            .addCase(getCategoryPlanList.fulfilled, (state, action: PayloadAction<HistoryOrPlaceCategryType[]>) => {
                state.loading = false
            ...
            .addCase(getTripPlanList.fulfilled, (state, action: PayloadAction<{ plan_list: HistoryType[], total_pages: number, current_page: number }>) => {
                state.loading = false
                let { plan_list, total_pages, current_page } = action?.payload || { plan_list: [], total_pages: 1, current_page: 1 };
...
export const getStatusLastRecommendationPlan = (state: RootState) => state.plan.statusLastRecommendationPlan;
export const getPlanRecommendationList = (state: RootState) => state.plan.planRecommendationList;
...
export const getPlanListPageSize = (state: RootState) => state.plan.planListPageSize;
export const getPlanListCurrentPage = (state: RootState) => state.plan.planListCurrentPage;
export const getPlanListTotalPages = (state: RootState) => state.plan.planListTotalPages;
export const getStatusPlantList = (state: RootState) => state.plan.statusPlanList;
export const getPlanList = (state: RootState) => state.plan.planList;
export const getPlanCategoryList = (state: RootState) => state.plan.planCategoryList;
export const getStatusPlanCategory = (state: RootState) => state.plan.statusCategoryPlan;
